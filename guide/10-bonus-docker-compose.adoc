...
+
[NOTE]
When starting in `spring.profiles.active`, each service will call the config server using the hostname `config-server` (instead of localhost). The actual name `config-server` will get defined in the `docker-compose.yml`, which we will create shortly.
+
...
Try it out by calling the application again and notice how the `item.serviceAddress` changes when the client loadbalancer does its work.
