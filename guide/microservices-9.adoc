...
|Finally we added tracing capabilities so we could keep track of the path a request to the system took. This can be used with tools like _Zipkin_ to gain performance statistics. An area we did not cover was log aggregation, which is another typical topic when it comes to tracing.
|===
