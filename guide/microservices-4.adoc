...
+
Each service will during startup automatically register to the Service Discovery server. The server will remove services when they shuts down (or if they time out).
...
It is very easy to register `items-services` and `reviews-services` with `service-discovery-server`. All that is needed is to add a dependency to _spring-cloud-starter-netflix-eureka-client_, and the Spring Cloud automagic will do the trick.
...
. Reload Eureka UI at http://localhost:8761/, both services should now have been registered properly in `service-discovery-server`.
____
