...
[NOTE]
====
The below curl examples pipes the result through `jq`, which is a lightweight JSON processor. It helps us formatting the output.
...
If you cannot install jq, just run below curl commands without `"| jq"`
====
