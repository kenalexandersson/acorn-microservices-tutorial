...
----
+
This means that if we start each service with `spring.profiles.active=docker`, they will startup using port 8080.
____
...
* The root `services` has several childs, one for each of our microservices. Each service will get the name specified here, the container of `config-server` will be named _config-server_, and so on. This will also act as the hostname internally in the docker environment.
...
* The `config-server`, `service-discovery-server`, `gateway` and `zipkin` maps their respective ports to 8080 (7777:8080, 8761:8080, 20202:8080 and 9411:8080) to the outside world. This allows us to connect via browsers or other clients by using _http://localhost:nnnn_. We are for example interested to still see the Eureka UI at http://localhost:8761, and also being able to access the application as usual from http://localhost:20202/webapi/items
